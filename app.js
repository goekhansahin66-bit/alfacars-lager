...
(function initSupabase() {
  const url = window.SUPABASE_URL || import.meta?.env?.SUPABASE_URL;
  const anonKey = window.SUPABASE_ANON_KEY || import.meta?.env?.SUPABASE_ANON_KEY;

  if (!url || !anonKey) {
    console.warn("⚠️ Supabase ENV fehlt – URL oder Key nicht gesetzt");
    return;
  }
  ...
  if (supabaseClient) {
    ...
    console.warn("⚠️ Supabase Client konnte nicht erstellt werden");
  }
})();
...
function saveOrder(){
  ...
  if(editingOrderId){
    ...
    orders.unshift({id:Date.now(),created:now(),status:"Bestellt",...data});
  }
