...
// ======================================================
// ULTRA-SAFE PATCH: verhindere Null-Crashes endgültig
// Überschreibt getElementById, damit fehlende Elemente
// niemals TypeErrors auslösen (NO-OP Dummy).
// ======================================================
(function(){
...
/* =========================================================
   EVENTS (robust & cache-safe)
   - FIX: Keine Crashes mehr, wenn ein Element (ID) fehlt
   - FIX: Alles erst nach DOMContentLoaded binden
   ========================================================= */

function overrideReadOnlyUI(){
  // Buttons, die Änderungen machen, auf Anzeige-Modus blockieren
  // FIX: btnExportAll ergänzt (war vorher nicht in der Liste)
  const ids = ["btnNew","btnSave","btnDelete","btnExportAll","btnNewCustomer","cbtnSave","cbtnDelete","btnNewStock","s_save","s_delete","btnDayClose","d_exportOrder","d_exportAll"];
  ids.forEach(id=>{
...
/**
 * FIX: Sicheres Event-Binding.
 * Verhindert: "Cannot set properties of null (setting 'onclick' / 'textContent')"
 */
function bind(id, event, handler){
  const el = document.getElementById(id);
  if (!el) return;      // Element existiert nicht -> kein Crash
  el.addEventListener(event, handler);
  return el;
}

function setupEvents(){
  // READ-ONLY Schutz (iPhone / ?ro=1)
  if (READ_ONLY) overrideReadOnlyUI();

  // Tabs (sicher)
  const tOrders = document.querySelector('[data-tab="orders"]');
  if (tOrders) tOrders.onclick = ()=>switchView("orders");
  const tArchive = document.querySelector('[data-tab="archive"]');
  if (tArchive) tArchive.onclick = ()=>switchView("archive");
  const tCustomers = document.querySelector('[data-tab="customers"]');
  if (tCustomers) tCustomers.onclick = ()=>switchView("customers");
  const tStock = document.querySelector('[data-tab="stock"]');
  if (tStock) tStock.onclick = ()=>switchView("stock");

  // Top actions
  bind("btnNew","click",openNewOrder);

  // ✅ Export-Button erzeugt jetzt EINE Excel-Datei mit Tabs
  bind("btnExportAll","click",exportExcelWorkbook);

  bind("btnSave","click",saveOrder); // ruft intern saveOrderToSupabase(currentOrder) auf
  bind("btnDelete","click",deleteOrder);
  bind("btnCancel","click",()=>$("modal")?.classList.add("hidden"));
  bind("btnClose","click",()=>$("modal")?.classList.add("hidden"));

  // Kunden
  bind("btnNewCustomer","click",()=>openCustomerModal(null));
  bind("cbtnSave","click",saveCustomerManual);
  bind("cbtnCancel","click",closeCustomerModal);
  bind("cbtnClose","click",closeCustomerModal);
  bind("cbtnDelete","click",deleteCustomer);

  // Lager
  bind("btnNewStock","click",openNewStock);
  bind("btnDayClose","click",openDayClose);
  bind("s_save","click",saveStockItem);
  bind("s_cancel","click",closeStockModal);
  bind("s_close","click",closeStockModal);
  bind("s_delete","click",deleteStockItem);

  // Tagesabschluss
  bind("d_close","click",closeDayClose);
  bind("d_cancel","click",closeDayClose);
  bind("d_exportOrder","click",exportDayCloseOrder);

  // ✅ “Komplettes Backup exportieren”
  bind("d_exportAll","click",exportExcelWorkbook);
  ...
  const si = bind("searchInput","input",()=>currentView==="orders"?renderOrders():renderArchive());
  // customer search
  bind("customerSearchInput","input",()=>{ renderCustomers(); });

  // Lager-Suche
  bind("stockSearchInput","input",()=>{ renderStock(); });

  // Formular / Kalkulation (sicher)
  // Order Modal calc
  ["f_qty","f_unit","f_deposit"].forEach(id=>{
    bind(id,"input",()=>{
      ...
      const ct = $("calc_total");
      const cr = $("calc_rest");
      if (ct) ct.textContent = money(t);
      if (cr) cr.textContent = money(Math.max(t-dep,0));
    });
  });

  bind("f_size","input",()=>{
    const el = $("f_size");
    if (el) el.value = normalizeTireSize(el.value);
  });

  // Stock Modal normalize + suggestions
  bind("s_size","input",()=>{
    const el = $("s_size");
    if (el) el.value = normalizeTireSize(el.value);
  });

  bind("s_brand","input",renderModelSuggestions);
  bind("s_season","change",renderModelSuggestions);
}

/* =========================================================
   INIT (robust & cache-safe)
   ========================================================= */

document.addEventListener("DOMContentLoaded", async ()=>{
  // FIX: Erst initialisieren, wenn DOM wirklich da ist
  setupEvents();
...
async function initApp(){
  await initOrdersFromSupabase(); // lädt orders[]
  switchView("orders");
}
